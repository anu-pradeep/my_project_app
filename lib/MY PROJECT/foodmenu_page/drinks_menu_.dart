...
class _drinkmenuState extends State<drinkmenu> {
  ...
  int index = 1;
  var drinks = ["Tea", "Coffee", "Bru coffee", "Green tea", "Lemon tea", "Bru coffee", "Lime juice", "Lime soda", "Mint lime", "Mojito", "Apple juice", "Mango juice", "Pineapple juice", "Strawberry juice", "Pappaya juice", "Cucumber juice", "chikku shake", "Falooda", "Avil milk", "Sharja shake"];

  TextEditingController searchController = TextEditingController();
  String searchQuery = "";

  List<String> get filteredDrinks {
    return drinks.where((drink) => drink.toLowerCase().contains(searchQuery.toLowerCase())).toList();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: CustomScrollView(
        slivers: [
          SliverAppBar(
            title: Center(child: Text("Hot & cool drinks", style: GoogleFonts.aclonica(color: Colors.black))),
          ...
          SliverList(
            delegate: SliverChildBuilderDelegate(
                ...
                return ListTile(
                  title: Text(filteredDrinks[index]),
                  trailing: Row(
                    ...
                    children: [
                      ElevatedButton.icon(
                        onPressed: () {
                          setState(() {
                            if (drinklist.contains(filteredDrinks[index])) {
                              drinklist.remove(filteredDrinks[index]);
                            } else {
                              drinklist.add(filteredDrinks[index]);
                            }
                          });
                        },
                        icon: Icon(Icons.favorite),
                        label: Text(
                          drinklist.contains(filteredDrinks[index]) ? "Remove" : "Add",
